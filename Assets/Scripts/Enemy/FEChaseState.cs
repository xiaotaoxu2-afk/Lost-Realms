...
public class FEChaseState : BaseState
{
    ...
    public override void LogicUpdate()
    {
        ...
        target = new Vector3(currentenemy.attacker.position.x, currentenemy.attacker.position.y + 1, 0);

        if (Mathf.Abs(target.x - currentenemy.attacker.position.x) <= attack.attackRange &&
            target.y - currentenemy.attacker.position.y <= attack.attackRange)
        {
            isAttack = true;
